package org.dbhaskaran.sweetshop.dao;
...
import org.dbhaskaran.sweetshop.dto.Recommend;
import org.dbhaskaran.sweetshop.utils.DBUtil;

public class GoodsDao {
	// select g.id,g.name,g.cover,g.price,t.name typename from recommend r,goods
	// g,type t where type=2 and r.goods_id=g.id and g.type_id=t.id
	...
	public List<Goods> selectSearchGoods(String keyword, int pageNumber, int pageSize) throws SQLException {
		...
		return r.query(sql, new BeanListHandler<Goods>(Goods.class), "%" + keyword + "%", (pageNumber - 1) * pageSize,
				pageSize);
	}
